...
export const UserProfile = ({ login, setAvatar }) => {
  ...
  useEffect(() => {
    const getImageURL = async () => {
      try {
        const photoPath = await getUserAvatar(login ?? userLogin);
        if (!photoPath) return;
  ...
  const onCopyId = () => {
    ...
    Clipboard.setStringAsync(userLogin ?? login);
  };
    ...
    <View style={{ alignItems: "center" }}>
      <View style={styles.profileContainer}>
        ...
        <TouchableOpacity onPress={onCopyId} style={styles.copyToClipboardButton}>
          <Text style={[styles.profileId, { color: "#FFF" }]}>{login ?? userLogin}</Text>
